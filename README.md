...
Although I know English, I am not an expert in its use, especially when it comes to conveying emotions. That is why the following text was written in my native language. Therefore, dear reader, if you do not know Polish, please use a translation tool or AI, which should best convey the content I want to present.
...
I to, co postrzegałem jako niesamowite w tejże postaci był fakt, że wszystko czego się nauczył i co opanował, dokonał tego samodzielnie. 
...
i jak wiele dzieciaków z tamtego czasu, miał poważny problem z ilością czasu jaki spędzał przy komputerze grając. Problem był istotny, bo syn miał komputer u siebie w pokoju i bywało,
...
Wracając do problemu z synem, zaprojektowałem aplikację oraz sposób jej działania, po czym napisałem kod - oczywiście nie od razu zadziałało jak chciałem - jednak koniec końców aplikacja powstała. 
...
wiedzę stworzyłem Guardiana w wersji 2.0. Zasada działania nowszej wersji była podobna do wersji 1.0 jednak z pewnym usprawnieniem. Aplikacja będąc w autostarcie Windowsa sprawdzała, czy jest już
...
dla overclockera, a gdy już się udało go zdobyć, niedługo po tym rozpoczęła się pandemia covid19 i było lepiej żeby młody siedział w domu i grał bez ograniczeń. W ten oto sposób moja aplikacja
...
projekt postanowiłem doprowadzać do końca za wszelką cenę, żeby ta nauka była wymierna. Testowałem wszelkie tematy, w których widziałem jakiekolwiek zastosowanie. Co oczywiście jako samouka
...
W tak zwanym międzyczasie rozwijając skille podstawowej biblioteki Pythona zostałem poproszony o napisanie, a właściwie o propozycję napisania aplikacji do sprawdzania numeru PESEL. Kod miał być częścią
...
z możliwością wpisania hasła, które pozwalałoby na dalsze korzystanie z komputera. Finalnie nie zastosowałem tego rodzaju rozwiązań, jednak ten sposób zabezpieczania aplikacji stał się trzonem
...
"You won't find my magic code" i należało zwrócić uwagę na słowo 'magic', które było trzeba wpisać w specyficzny sposób. Zachęcam do sprawdzenia. Miłej zabawy :)
...
zachęcam do samodzielnego wypróbowania. Windows 11 posiada już wbudowaną obsługę kolorów ANSI wykorzystaną w tym projekcie, więc nie trzeba już zaglądać do Linuxowego terminala, który mimo wszystko odstrasza
...
porażką. Robiłem tak wiele razy już kiedyś, i zrobię w przyszłości - o czym napiszę poniżej. W każdym bądź razie organizator konkursu próbował wspierać mnie w moim przedsięwzięciu sugerując żebym się nie poddawał.
...
Pan Maciej był dla mnie swoistym autorytetem i ekspertem uwierzyłem w jego słowa podkręcane zresztą przez mój brak wiary w siebie. Tu niestety mój talizman nie wykazywał mocy. Moje własne przekonania miały większą siłę.  I ponownie 
uznałem - pomimo że programowanie sprawia mi niesamowitą frajdę - że nie powinienem się nim zajmować. Że od tego są wykształceni specjaliści. I te słowa w pewnym momencie ugasiły żar moich marzeń.
...
przyjąłem ten zawód z całą jego specyfiką. Miesięcznie pokonuję dystans rzędu od ośmiu do jedenastu tysięcy kilometrów jeżdżąc w znacznej większości nocami. Praca jest wymagająca. I posiada pewną istotną cechę, która pomogła 
...
Aby odnaleźć siebie chociaż w tej części, w moim przypadku musiało minąć dużo czasu. Nie wiem tak naprawdę skąd brała się we mnie ta iskierka, która pchała mnie w kierunku programowania pomimo problemów zapoczątkowanych pod koniec poprzedniego
...
brzemię. Z jednej strony nakręca do robienia rzeczy lepiej, ale z drugiej nigdy nie daje zadowolenia z ich jakości. Perfekcjonizm, bo o nim mowa - towarzyszy mi odkąd pamiętam. I nie ważne co robiłem, zazwyczaj nie byłem zadowolony z efektów sądząc że 
...
I pomimo że moje aplikacje działały, ja widziałem w nich tylko niedoróbki. To była główna przyczyna, dlaczego wszystko co tworzyłem trafiało do szuflady. Wstydziłem się uważając że każdy projekt nie jest idealny, więc nie jest warty pokazania.
...
trwały w nieskończoność. I przez to zachowanie nie widziałem wartości w aplikacjach, które wychodziły spod moich palców. A jakby nie patrzeć projektowałem aplikacje, tworzyłem sprawny i działający kod, który budowałem i dbałem o sprawne uruchomienie. 
A to zdaje się jest dokładnie tym, czym zajmują się zawodowi programiści. Ja nie widziałem wartości swoich pomysłów, bo nie mialem zewnętrznego punktu odniesienia, że samodzielnie robię to, co w realnym świecie całe zespoły. 
...
Kończę tę odyseję spostrzeżeniem, że tak naprawdę nie była to historia o nauce programowania. Programowanie było tylko językiem, w którym ta historia się wydarzyła.
